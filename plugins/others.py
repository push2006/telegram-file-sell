...
from hydrogram.errors.pyromod import ListenerTimeout
...
async def home(client: Client, query: CallbackQuery):
    from plugins.start import start_inline_kb
    buttons = start_inline_kb(query.from_user.id in client.admins)
    await query.message.edit_text(
        ...
        ) + "\n\n🛒 Tap <b>Catalog</b> to browse packs.",
...
async def about(client: Client, query: CallbackQuery):
    buttons = [
        ...
        [InlineKeyboardButton("Back", callback_data = "home"), InlineKeyboardButton("Close", callback_data = "close")],
    ]
    await query.message.edit_text(
...
@Client.on_message(filters.private & filters.regex(r"^Help$"))
async def help_keyboard(client: Client, message: Message):
    buttons = [
        [InlineKeyboardButton("🛒 Catalog", callback_data="open_catalog")],
        [InlineKeyboardButton("Close", callback_data="close")],
    ]
    await message.reply_text(
        text=client.messages.get('ABOUT', 'Help').format(
            owner_id=client.owner,
