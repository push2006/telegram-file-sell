...
async def ensure_subscribed(client, chat_id: int, user_id: int) -> bool:
    """Reusable subscription gate for handlers that aren't a plain `Message`
    command (callback queries, or message handlers that shouldn't carry the
    "wait a second..." placeholder that `force_sub` shows).

    Returns True if the user is subscribed (or no fsub is configured).
    Otherwise sends the "please subscribe" prompt to chat_id and returns False.
    """
    ...
    # Always give a way back in, even with no deep-link payload to replay.
    buttons.append(InlineKeyboardButton("🔄 Try Again", url=f"https://t.me/{client.username}"))

    buttons_markup = InlineKeyboardMarkup([[button] for button in buttons])
    try:
        await client.send_message(chat_id, text=channels_message, reply_markup=buttons_markup)
    except Exception as e:
        client.LOGGER(__name__, client.name).warning(f"Error sending force sub prompt: {e}")
    return False
...
def force_sub(func):
    ...
    async def wrapper(client: Client, message: Message):
        ...
        # Add "Try Again" button — always, so users who /started with no
        # payload can re-trigger the check after joining instead of getting stuck.
        from_link = message.text.split(" ")
        if len(from_link) > 1:
            try_again_link = f"https://t.me/{client.username}/?start={from_link[1]}"
        else:
            # No payload to replay — just reopen the bot so the user can send /start again.
