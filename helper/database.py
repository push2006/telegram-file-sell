...
class MongoDB:
    ...
    def __new__(cls, uri: str, db_name: str):
        if (uri, db_name) not in cls._instances:
            ...
            instance.categories = instance.db['categories']    # Shop categories (paid content groups)
            instance.coupons = instance.db['coupons']          # Redeemable coupon codes
            instance.category_access = instance.db['category_access']  # user -> category unlocks
            instance.orders = instance.db['orders']            # payment orders (stars/card/crypto)
    ...
    # ---------------- Shop: categories, coupons, access, orders ----------------

    async def create_category(self, name: str, price_cents: int, access_days: int = 0) -> str:
        """Create a paid/free content category. access_days=0 means lifetime access."""
        res = await self.categories.insert_one({
            "name": name,
    ...
    async def get_category(self, category_id) -> dict:
        ...
        return await self.categories.find_one({"_id": ObjectId(category_id)})

    async def get_category_by_name(self, name: str) -> dict:
        return await self.categories.find_one({"name": name})
    ...
    async def grant_category_access(self, user_id: int, category_id, expiry_date: datetime = None):
        await self.category_access.update_one(
            {"user_id": user_id, "category_id": str(category_id)},
            {"$set": {"user_id": user_id, "category_id": str(category_id), "expiry_date": expiry_date, "granted_at": datetime.now()}},
            upsert=True
        )

    async def has_category_access(self, user_id: int, category_id) -> bool:
        doc = await self.category_access.find_one({"user_id": user_id, "category_id": str(category_id)})
        if not doc:
            return False
    ...
    async def create_coupon(self, code: str, category_id, uses_left: int = 1, access_days: int = 0) -> bool:
        try:
            await self.coupons.insert_one({
    ...
    async def redeem_coupon(self, code: str, user_id: int) -> dict:
        """Atomically consume one use of a coupon. Returns {'ok': bool, 'category_id'|'error'}."""
        code = code.strip().upper()
        doc = await self.coupons.find_one_and_update(
            {"code": code, "uses_left": {"$gt": 0}},
            {"$inc": {"uses_left": -1}}
        )
        if not doc:
            return {"ok": False, "error": "Invalid or already-used coupon code."}

        expiry_date = None
        if doc.get("access_days"):
            expiry_date = datetime.now() + timedelta(days=doc["access_days"])
        await self.grant_category_access(user_id, doc["category_id"], expiry_date)
        return {"ok": True, "category_id": doc["category_id"]}

    async def create_order(self, user_id: int, category_id, amount_cents: int, method: str, status: str = "pending") -> str:
        res = await self.orders.insert_one({
            "user_id": user_id,
            "category_id": str(category_id),
            "amount_cents": amount_cents,
            "method": method,       # "stars" | "card" | "crypto"
            "status": status,       # "pending" | "paid" | "confirmed" | "rejected"
            "created_at": datetime.now()
        })
        return str(res.inserted_id)

    async def mark_order_paid(self, order_id: str, status: str = "paid"):
        from bson import ObjectId
        await self.orders.update_one(
            {"_id": ObjectId(order_id)},
            {"$set": {"status": status, "paid_at": datetime.now()}}
        )
    ...
    async def get_pending_orders(self) -> list:
        return await self.orders.find({"status": "pending"}).to_list(length=None)

    async def clear_shop_data(self, mode: str = "orders") -> dict:
        """Wipe shop test data. mode: orders | access | all
        - orders: pending/paid orders only
        - access: orders + unlocks + memberships + coupons
        - all: above + categories (packs)
        """
        counts = {}
        counts["orders"] = (await self.orders.delete_many({})).deleted_count
        if mode in ("access", "all"):
            counts["category_access"] = (await self.category_access.delete_many({})).deleted_count
            counts["channel_memberships"] = (
                await self.db["channel_memberships"].delete_many({})
            ).deleted_count
            counts["coupons"] = (await self.coupons.delete_many({})).deleted_count
        if mode == "all":
    ...
    async def add_category_content_item(self, category_id, chat_id: int, message_id: int,
                                          name: str = "", is_free_trial: bool = False) -> str:
        import uuid
        from bson import ObjectId
        ...
        await self.categories.update_one(
            ...
            {"$push": {"content": {
                ...
                "name": name, "is_free_trial": is_free_trial, "active": True
            }}}
        )
        return item_id

    async def list_content_items(self, category_id) -> list:
        cat = await self.get_category(category_id)
        if not cat:
            return []
        return [c for c in cat.get("content", []) if c.get("active", True)]

    async def deactivate_content_item(self, category_id, item_id: str) -> bool:
        from bson import ObjectId
        res = await self.categories.update_one(
            {"_id": ObjectId(category_id), "content.item_id": item_id},
            {"$set": {"content.$.active": False}}
        )
        return res.modified_count > 0

    async def has_used_free_trial(self, user_id: int, item_id: str) -> bool:
        doc = await self.db["free_trials"].find_one({"user_id": user_id, "item_id": item_id})
        return doc is not None

    async def mark_free_trial_used(self, user_id: int, item_id: str):
        await self.db["free_trials"].update_one(
            {"user_id": user_id, "item_id": item_id},
            {"$set": {"user_id": user_id, "item_id": item_id, "used_at": datetime.now()}},
            upsert=True
        )

    async def set_category_channel(self, category_id, channel_id: int):
        from bson import ObjectId
        await self.categories.update_one(
            {"_id": ObjectId(category_id)},
            {"$set": {"access_channel_id": channel_id}}
        )

    async def save_channel_membership(self, user_id: int, category_id, channel_id: int,
                                       invite_link: str, expires_at: datetime = None):
        await self.db["channel_memberships"].update_one(
            {"user_id": user_id, "category_id": str(category_id)},
            {"$set": {
                "user_id": user_id, "category_id": str(category_id), "channel_id": channel_id,
                "invite_link": invite_link, "expires_at": expires_at, "kicked": False,
                "granted_at": datetime.now()
            }},
            upsert=True
        )

    async def list_expired_memberships(self) -> list:
        return await self.db["channel_memberships"].find({
            "kicked": False,
            "expires_at": {"$ne": None, "$lt": datetime.now()}
        }).to_list(length=None)

    async def mark_membership_kicked(self, user_id: int, category_id):
        await self.db["channel_memberships"].update_one(
            {"user_id": user_id, "category_id": str(category_id)},
            {"$set": {"kicked": True}}
        )

    async def search_categories(self, keyword: str) -> list:
        cursor = self.categories.find({"name": {"$regex": keyword, "$options": "i"}, "active": True})
        return await cursor.to_list(length=None)

    async def get_user_unlocked_categories(self, user_id: int) -> list:
        access_docs = await self.category_access.find({"user_id": user_id}).to_list(length=None)
        from bson import ObjectId
        cats = []
        for doc in access_docs:
            if doc.get("expiry_date") and doc["expiry_date"] < datetime.now():
                continue
            try:
